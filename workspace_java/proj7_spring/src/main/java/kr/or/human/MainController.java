...
// Controller Bean (즉 spring이 생성할 수 있는 class)로 등록한다
@Controller
...
public class MainController {
	...
	//메소드에 적용한 @RequestMapping은 HandlerAdaptor가 처리합니다
	@RequestMapping("/test/main1.do") //괄호 안에 내가 받고 싶은 주소 넣기
	public ModelAndView main1() {
		System.out.println("main1 실행");
		
		//뷰리졸버에 주기 위해서 뷰네임에 담는 과정? 
	...
	@RequestMapping("/test/main2.do") //괄호 안에 내가 받고 싶은 주소 넣기
	public ModelAndView main2() {
		...
		//보내고 싶은 것을 request.setAttribute 처럼 담을 수 있는 방법
		mav.addObject("msg","홍길동");
		
		return mav;
	}
	
	// 전달인자가 value 하나일때는 value라는 글씨를 생략할 수 있다
	// 두 개 이상 값을 넣을 때는 {} 안에 넣는다
	// value는 url 패턴을 뜻한다
	// method는 접근 방식 제한함
	// 	적은 것만 들어올 수 있음
...
//	}
	...
	//이거랑 아래 post방식은 다른 거임
	// url은 같지만 method가 달라서 가능
	@RequestMapping(value="/test/main5.do", method=RequestMethod.GET)
//	@GetMapping("/test/main5.do") 이렇게 축약해서 사용가능
